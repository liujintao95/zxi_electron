<template>
  <div class="download">
    <div>
      <el-button-group>
        <el-button @click="selectionStart" icon="el-icon-download" size="small"
          >下载</el-button
        >
        <el-button
    ...
    <div style="height: 100%">
      ...
      >
        ...
        <el-table-column prop="size_fmt" label="大小" width="120">
        </el-table-column>
        ...
        <el-table-column label="操作" width="150" align="center">
          <template slot-scope="scope">
            <el-button
              v-if="!scope.row['downloading'] && !scope.row['is_complete']"
            ...
            >
            <el-button
              v-else-if="scope.row['downloading'] && !scope.row['is_complete']"
...
export default {
  ...
  methods: {
    ...
    downloadZXiFile(download_id, file_name, local_path) {
      this.showDownloadInfo(download_id)
        .then((data) => {
          ...
          if (download["is_complete"] === 0) {
            ...
            this.interval_map[download_id] = setInterval(
              this.showProgress,
              this.timeout,
              download_id
            );
    ...
    startDownload(row) {
      if (!row["downloading"] && !row["is_complete"]) {
        this.$axios
          .put(`/zxi/auth/download/start/${row["id"]}`)
          .then(() => {
            this.showDownloadTable();
          })
          .catch((error) => {
    ...
    pauseDownload(row) {
      if (row["downloading"] && !row["is_complete"]) {
        this.$axios
          .put(`/zxi/auth/download/pause/${row["id"]}`)
          .then(() => {
    ...
    cancelDownload(row) {
      if (!row["is_complete"]) {
        this.$axios
          .put(`/zxi/auth/download/cancel/${row["id"]}`)
          .then(() => {
            if (row["downloading"]) {
              clearInterval(this.interval_map[row["id"]]);
            }
    ...
    showProgress(download_id) {
      this.$axios
        .get(`/zxi/auth/download/progress/${download_id}`)
        .then((response) => {
