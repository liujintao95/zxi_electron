<template>
  <div class="upload">
    <div>
      <el-button-group>
        <el-button @click="selectionStart" icon="el-icon-upload2" size="small"
          >开始</el-button
        >
        <el-button
          @click="selectionPause"
          icon="el-icon-video-pause"
          size="small"
          >暂停</el-button
        >
        <el-button @click="selectionCancel" icon="el-icon-delete" size="small"
          >取消</el-button
        >
      </el-button-group>
    ...
    <div style="height: 100%">
      ...
      >
        ...
        <el-table-column label="操作" width="150" align="center">
          <template slot-scope="scope">
            <el-button
              v-if="!scope.row['uploading'] && !scope.row['is_complete']"
            ...
            <el-button
              v-else-if="scope.row['uploading'] && !scope.row['is_complete']"
  ...
  methods: {
      ...
      this.showUploadInfo(upload_id)
        .then((data) => {
          ...
          if (upload["is_complete"] === 0) {
            ...
            this.interval_map[upload_id] = setInterval(
              this.showProgress,
              this.timeout,
              upload_id
            );
    ...
    uploadCheck() {
      for (let upload_info of this.tableData) {
        if (
          upload_info["uploading"] &&
          !upload_info["is_complete"] &&
          !this.$stream_map[upload_info["id"]]
        ) {
          this.uploadLocalFile(
            upload_info["id"],
            upload_info["name"],
            upload_info["local_path"]
          );
        }
      }
    },
    ...
    startUpload(row) {
      if (!row["uploading"] && !row["is_complete"]) {
        this.$axios
          .put(`/zxi/auth/upload/start/${row["id"]}`)
          .then(() => {
            this.showUploadTable();
          })
          .catch((error) => {
    ...
    pauseUpload(row) {
      if (row["uploading"] && !row["is_complete"]) {
        this.$axios
          .put(`/zxi/auth/upload/pause/${row["id"]}`)
          .then(() => {
    ...
    cancelUpload(row) {
      if (!row["is_complete"]) {
        this.$axios
          .put(`/zxi/auth/upload/cancel/${row["id"]}`)
          .then(() => {
            if (row["stream"]) {
              row["stream"].emit("end");
              clearInterval(this.interval_map[row["id"]]);
    ...
    saveFile(path) {
      this.$file.getFilesList(path).then((data) => {
        ...
        this.$axios
          .post("/zxi/auth/upload/create", form)
          .then(() => {
            this.cpage = 1;
            this.showUploadTable();
          })
    ...
    saveDir(path) {
      this.$file.getFilesList(path).then(async (data) => {
        for (let file_info of data) {
          ...
          try {
            await this.$axios.post("/zxi/auth/upload/create", form);
            this.cpage = 1;
            this.showUploadTable();
          } catch (err) {
            console.log(err);
    ...
    async showUploadInfo(upload_id) {
      let response = await this.$axios.get(
        `/zxi/auth/upload/info/${upload_id}`
      );
      return response.data;
    },
    showUploadTable() {
      this.$axios
        .get("/zxi/auth/upload/show", {
          params: {
            page: this.cpage,
            size: this.page_size,
          },
    ...
    showProgress(upload_id) {
      this.$axios
        .get(`/zxi/auth/upload/progress/${upload_id}`)
        .then((response) => {
          ...
          for (let i = 0; i < this.tableData.length; i++) {
            if (this.tableData[i]["id"] === upload_id) {
              if (parseInt(progress) === 100) {
                ...
                if (diff_value > 0) {
                  let rate =
                    ((diff_value / 100) * this.tableData[i]["size"]) /
                    (this.timeout / 1000);
